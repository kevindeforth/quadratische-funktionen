...
\begin{exercise}\label{ex:quad_eq_allgemein_params}
...
\begin{exercise}\label{ex:quad_eq_allgemein_einfluss_params}
...
\begin{exercise}\label{ex:quad_eq_allgemein_einfluss_param_c}
...
\begin{exercise}\label{ex:quad_func_zeichnen}
...
Eine quadratische Gleichung ist eine Gleichung mit Unbekannter $x$, die sich in der Form \begin{IEEEeqnarray}{c}
ax^2 + bx + c = 0\label{eq:quad_eq_allgemein}
\end{IEEEeqnarray}
schreiben lässt, wobei $a,b$ und $c$ Koeffizienten sind und $a\neq 0$ gelten muss. Im Folgenden wird davon ausgegangen, dass die Koeffizienten $a,b$ und $c$ reelle Zahlen sind.

Sie erkennen wahrscheinlich, dass die Lösungsmenge der Gleichung~\ref{eq:quad_eq_allgemein} genau den Nullstellen der quadratischen Funktion in allgemeiner Form entspricht \realFun{f}{x}{ax^2 + bx + c} 
\printFunDef*{f}.
...
Wie bei den quadratischen Funktion nennt man die Schreibweise mit dem Quadratkoeffizienten $a$, dem Linearkoeffizienten $b$ und der Konstanten $c$ eine Quadratische Gleichung in \emph{allgemeiner Form}.

Eine Gleichung der Form
\begin{IEEEeqnarray}{c}
a(x-d)^2 + e = 0\label{eq:quad_vertex}
\end{IEEEeqnarray}

ist ebenfalls eine quadratische Gleichung und man nennt diese eine quadratische Gleichung in \emph{Scheitelpunktform}.

Sie wissen bereits, die Lösungsmenge der Gleichung~\ref{eq:quad_vertex} der Menge der Nullstellen der Quadratischen Funktion mit Quadratkoeffizient $a$ und Scheitelpunkt $(d,e)$ entspricht. Sie lässt sich wie folgt auflösen:
...
\iff & a(x-d)^2 &=& -e\\
\iff & (x-d)^2 &=& -\frac{e}{a}\\
\iff &x-d &=& \pm \sqrt{-\frac{e}{a}}\\
\iff &x &=& d \pm \sqrt{-\frac{e}{a}}.\label{eq:sol_quad_eq_vertex}
\end{IEEEeqnarray}
Die Quadratische Gleichung hat entsprechend:
\begin{itemize}
\item Keine reelle Lösung, wenn $e$ und $a$ das gleiche Vorzeichen haben;
\item Eine reelle Lösung, wenn $e$ gleich Null ist. In diesem Fall lautet die Lösungsmenge $\lbrace d\rbrace$;
\item Genau zwei reelle Lösungen, wenn $e$ und $a$ unterschiedliche Vorzeichen haben. In diesem Fall lautet die Lösungsmenge $\lbrace d + \sqrt{-\frac{e}{a}}, d - \sqrt{-\frac{e}{a}}\rbrace$
\end{itemize}

Intuitiv lässt sich das so erklären: wenn $e$ gleich Null ist, dann liegt der Scheitelpunkt auf der Abszisse (der x-Achse) und entsprechend entspricht der Scheitelpunkt auch dem einzigen Nullpunkt der quadratischen Funktion \realFun{g}{x}{a(x-d)^2+e}\printFunDef*{g}. Wenn $e$ und $a$ unterschiedliche Vorzeichen haben, dann ist der Funktionsgraph entweder nach oben geöffnet und der Scheitelpunkt liegt unterhalb der Abszisse oder der Funktionsgraph ist nach unten geöffnet und der Scheitelpunkt liegt oberhalb der Abszisse. In beiden Fällen muss der Funktionsgraph die Abszisse genau zwei mal schneiden, entsprechend müssen zwei Nullstellen existieren.
Wenn $a$ und $e$ das gleiche Vorzeichen haben, dann schneidet der Funktionsgraph niemals die Abszisse.

Zuletzt gilt es die \emph{Normalform} der quadratischen Gleichung vorzustellen. Diese lautet wie folgt:
\begin{IEEEeqnarray}{c}
x^2 + px + q = 0
\end{IEEEeqnarray}

\begin{exercise}
Formen Sie die allgemeine quadratische Form so um, dass Sie die Normalform erhalten. Wie lauten die Parameter $p$ und $q$?
\end{exercise}


Die allgemeine Lösungsformel für quadratische Gleichungen der Form~\ref{eq:quad_eq_allgemein} lässt sich finden, indem wir in Gleichung~\ref{eq:sol_quad_eq_vertex} $d$ und $e$ mit den Koordinaten des Scheitelpunktes ersetzen:
...
\iff & x_{1,2} &=& \frac{-b\pm\sqrt{b^2 - 4ac}}{2a}.\label{eq:quad_eq_general_sol}
\end{IEEEeqnarray}
\newpage
\subsection{Übungen}
\begin{exercise}\label{ex:identify_quad_eqs}
Bestimmen Sie, welche der folgenden Gleichungen quadratisch sind.
\begin{multicols}{2}
\begin{enumerate}[label=\alph*)]
\item $3x^{2}=4$;
\item $-5x^{2}+2x+5x^{2}=1$;
\item $y^{2}=0$;
\item $ax+7=2$;
\item $n^{3}+4n^{2}+2n+1=0$;
\item $13x-14=2x^{2}$;
\item $3=2y^{2}-6y$;
\item $\frac{1}{2}n^{2}=3+\frac{1}{7}n$;
\item $\sqrt{2}x^{2}+3=0$;
\item $(y+2)(y-4)=0$;
\item $2(x+7)=0$;
\item $x^{2}+2x=(x+1)^{2}-6$.
...
\begin{exercise}\label{ex:complete_square_solve_eqs}
Vervollständigen Sie das Quadrat, um folgende quadratische Gleichungen nach $y$ aufzulösen.
\begin{multicols}{2}
\begin{enumerate}[label=\alph*)]
\item $y^{2}-3y+2=0$;
\item $5y^{2}=20$;
...
\begin{exercise}\label{ex:solve_quad_eq_formula}
Benutzen Sie die quadratische Formel aus Gleichung~\ref{eq:quad_eq_general_sol} um die folgenden Gleichungen nach $y$ aufzulösen.
\begin{multicols}{2}
\begin{enumerate}[label=\alph*)]
\item $y^{2}-3y+2=0$;
\item $5y^{2}=20$;
...
\begin{exercise}\label{ex:lueckentext_quad_eq_diskriminante}
Vervollständigen Sie den Text:
Es sei die quadratische Gleichung $ax^2 + bx + c=0$ und es sei $\Delta=b^{2}-4ac$. 
\begin{enumerate}[label=\alph*)]
\item Wenn $\Delta>0$, dann gibt es genau \rule{1cm}{0.5pt} Lösungen.
\item Wenn $\Delta=0$, dann gibt es genau \rule{1cm}{0.5pt} Lösungen.
\item Wenn $\Delta<0$, , dann gibt es genau \rule{1cm}{0.5pt} Lösungen.
\end{enumerate}
\end{exercise}
...
\begin{exercise}\label{ex:quad_gleich_system}
Wenn $-2$ eine Lösung der quadratischen Gleichung $x^{2}-17x+c=0$ ist, was sind dann die Lösungen zur quadratischen Gleichung $x^{2}+13x+c=0$?
\end{exercise}



\begin{exercise}\label{ex:quad_gleich_gemeinsame_loesung}
Bestimmen Sie die Werte $c,$ für welche die Gleichungen $6x^{2}-17x+12=0$ und $3x^{2}-2x+c=0$ eine gemeinsame Lösung haben.
\end{exercise}


\subsubsection{Satz von Vieta (anspruchsvollere Aufgaben)}
\begin{exercise}\label{ex:vieta}
\begin{enumerate}[label=\alph*)]
\item Es sei die quadratische Gleichung $x^{2}+px+q=0$ und es seien $x_1$ und $x_2$ ihre Lösungen. Bestimmen Sie das Produkt $x_1 \cdot x_2$ und die Summe $x_1 + x_2$ \emph{\footnotesize (Hinweis: nutzen Sie die quadratische Lösungsformel aus Gleichung~\ref{eq:quad_eq_general_sol} um $x_1$ und $x_2$ zu bestimmen)}.
\item Bestimmen Sie eine quadratische Gleichung, deren Lösungen:
\begin{enumerate}[label=\roman*)]
\item $-1$ und $4$;
\item $-2$ und $2$;
\item $3$ und $3$ sind.
\end{enumerate}
\item Der französische Mathematiker François Vieta behauptet, dass die Lösungen $\alpha$ und $\beta$ der quadratischen Gleichung $ax^{2}+bx+c=0$ folgende Beziehungen erfüllen:
\begin{enumerate}[label=\roman*)]
\item $\alpha+\beta=-\frac{b}{a}$
\item $\alpha\cdot \beta=\frac{c}{a}$
\end{enumerate}
Würden Sie François zustimmen? Weshalb? Können Sie es formal beweisen?
\item Beweisen Sie, dass wenn zwei reelle Zahlen $\alpha, \beta$ folgende Beziehungen erfüllen:
\begin{enumerate}[label=\roman*)]
\item $\alpha+\beta=-\frac{b}{a}$
\item $\alpha\cdot \beta=\frac{c}{a},$
\end{enumerate}
Dass $\alpha$ und $\beta$ dann Lösungen zur Gleichung $ax^2 + bx + c = 0$ sein müssen.
\end{enumerate}
\end{exercise}
...
\begin{exercise}\label{ex:vieta_applied}
Es seien $\alpha$, $\beta$ Lösungen der quadratischen Gleichung $ax^{2}+bx+c=0$, $a,b,c \in \mathbb{R}$ mit $a\neq 0$ und $c\neq 0$. Nutzen Sie die Ergebnisse aus Aufgabe~\ref{ex:vieta} um zu beweisen, dass die quadratische Gleichung $cx^{2}+bx+a=0$ (genannt \emph{reziprok} zu $ax^{2}+bx+c=0$) die Lösungen $\frac{1}{\alpha}$ und $\frac{1}{\beta}$ hat.
\end{exercise}
...
%
%\begin{exercise}
%\underline{Solving quadratic equations through factorisation}: Determine an algorithm to write $ax^{2}+bx+c$, $a,b,c \in \mathbb{R}$ with $a\neq 0$, as a product of two linear factors i.e. determine $d,e,f,g \in \mathbb{R}$ such that 
%$$ax^{2}+bx+c=(dx+e)(fx+g).$$
%Hint 1: consider the case $c=0$.\\
%Hint 2: now assume $c\neq 0$. Expand $(dx+e)(fx+g)$ and let $\alpha=ef$ and $\beta=dg$.  Determine the relations between $a,b,c$ and $\alpha$ and $\beta$. \\
%Alternatively: write $ax^{2}+bx+c=a(x^{2}+\frac{b}{a}x+\frac{c}{a})$ and determine $n, m\in \mathbb{R}$ such that $a(x^{2}+\frac{b}{a}x+\frac{c}{a})=a(x+n)(x+m)$.
%\end{exercise}
%
%\begin{exercise}
...
%
%\begin{exercise}
%\underline{Solving quadratic equations through completion to square}: Determine an algorithm to write $ax^{2}+bx+c$, $a,b,c \in \mathbb{R}$ with $a\neq 0$, as $(dx+e)^{2}=g$ where $d,e,g\in \mathbb{R}$.
%\end{exercise}
%
%
%\begin{exercise}
%\underline{Solving quadratic equations using the quadratic formula}: Show that the roots of $ax^{2}+bx+c=0$, $a,b,c \in \mathbb{R}$ with $a\neq 0$,  are $\frac{-b\pm \sqrt{b^{2}-4ac}}{2a}$.
%\end{exercise}
%
%
%
...
%
%\begin{exercise}
%Show that $0$ is a root of  $ax^{2}+bx+c=0$, $a,b,c \in \mathbb{R}$ with $a\neq 0$ if and only if $c=0$.
%\end{exercise}
%
%
%
%\begin{exercise}
%Show that for each pair of real numbers $(\alpha, \beta)$, there exists an infinity of quadratic equation that admits them as roots. 
%\end{exercise}
%
%
%
%\begin{exercise}
%\begin{enumerate}[label=\alph*)]
%\item Find a quadratic equation with rational coefficients that admits $\frac{1}{2+\sqrt{5}}$ as a root. 
%\item \underline{Generalisation}: Let $ax^{2}+bx+c=0$, $a,b,c \in \mathbb{Q}$ with $a\neq 0$, be a quadratic equation with irrational roots $\alpha$ and $\beta$. Show that $\alpha$ and $\beta$ are conjugate pairs. \\
%\underline{Recall}. Let $q\in \mathbb{R}\setminus \mathbb{Q}$. Then $q=a+\sqrt{b}$ where $a\in \mathbb{Q}$ and $\sqrt{b}\in \mathbb{R}\setminus \mathbb{Q}$. The irrational number $q'=\frac{1}{q}=\frac{a-\sqrt{b}}{a^{2}-b^{2}}$ is called the conjugate of $q$; and $q$ and $q'$ are called a conjugate pair.
%\end{enumerate}
%\end{exercise}
%
%
%
%\begin{exercise}
%Find the values of $c$ for which the equations $x^{2}-11x+c=0$ and $x^{2}-14x+2c=0$ have a common root.
%\end{exercise}
%
%\begin{exercise}
%Outline an algorithm to solve the following equations by reducing them to quadratic equations. (You do not have to find explicit solutions, just to explain how would one arrive at those solutions).
%\begin{enumerate}[label=\alph*)]
%\item $ax^{4}+bx^{2}+c$, where $a,b,c\in \mathbb{R}$ with $a\neq 0$.
%\item $a\cdot p(x)^{2}+b\cdot p(x)+c=0$, where $a,b,c\in \mathbb{R}$ with $a\neq 0$, and $p(x)$ is an expression in $x$. 
%\item $(x+a)(x+b)(x+c)(x+d)+e=0$, where $a,b,c,d,e\in \mathbb{R}$ with $a+b=c+d$. 
%\end{enumerate}
%\end{exercise}
\newpage
\section{Lösungen}
...
\end{solution}
\begin{solution}[ex:einfuehrung_lin_comb_func]
\footnotesize
Wir haben 
\begin{IEEEeqnarray*}{rCrClCrCrClCrCrCl}
f &:& \mathbb{R} &\rightarrow& \mathbb{R}, &\qquad \qquad& g &:& \mathbb{R} &\rightarrow & \mathbb{R} & \qquad \qquad & (f+g) &:& \Reals & \rightarrow & \Reals \\
&& x&\mapsto& -2x+5 
&&
&& y &\mapsto& 0.5y-10
&&
...
\begin{IEEEeqnarray*}{rCrCl}
(v-u) &: & ]-\infty, -1[ \cup [1, +\infty[ & \rightarrow & \Reals\\
& & x & \mapsto & v(x) - v(x) = \frac{1}{x} - (1+x) = \frac{1}{x} - x - 1 
\end{IEEEeqnarray*}
...
Für negative $x$ funktioniert die Argumentation ebenfalls, für diese gilt dann aber dass $-\frac{1}{N_x+1} \geqslant x > -\frac{1}{N_x}$ und $\frac{1}{x} < \frac{1}{-\frac{1}{N_x}} = -N_x$.
\end{enumerate}
\end{enumerate}
...
(f \cdot g) &: & \Reals & \rightarrow & \Reals &&\\
& & x & \mapsto & f(x) \cdot g(x) &=& (-2x+5) \cdot (0.5x-10)\\
&&&&&=& -2x(0.5x-10) + 5(0.5x-10)\\
&&&&&=& -x^2 +22.5x -50
\end{IEEEeqnarray*}
\item Der Funktionswert von $g$ ist Null genau dann, wenn das Funktionsargument gleich $20$ ist: $g(20) = 0.5\cdot 20 - 10 = 0$. Entsprechend passen wir die Definitionsmenge an:

\begin{IEEEeqnarray*}{rCrClCr}
(\frac{f}{g}) &: & \Reals \setminus \{20\} & \rightarrow & \Reals &&\\
& & x & \mapsto & \frac{f(x)}{g(x)} &=& \frac{-2x+5}{0.5x-10}
\end{IEEEeqnarray*}
...
& & x & \mapsto & u(x) \cdot v(x) &=& (1+x)\cdot (\frac{1}{x}) = \frac{1}{x} + 1\\
\end{IEEEeqnarray*}
\item Der Funktionswert von $v$ beträgt nie Null, deshalb finden wir:
\begin{IEEEeqnarray*}{rCrClCr}
\frac{u}{v} &: & \Reals \setminus \{0\} & \rightarrow & \Reals &&\\
& & x & \mapsto & \frac{u(x)}{v(x)} &=& \frac{1+x}{\frac{1}{x}} = x^2 + x\\
\end{IEEEeqnarray*}
\item Der Funktionswert von $u$ beträgt Null im Punkt $-1$. Somit finden wir:
\begin{IEEEeqnarray*}{rCrClCr}
\frac{v}{u} &: & \Reals \setminus \{0, -1\} & \rightarrow & \Reals &&\\
& & x & \mapsto & \frac{v(x)}{u(x)} &=& \frac{\frac{1}{x}}{1+x} = \frac{1}{x^2 + x}\\
\end{IEEEeqnarray*}
\end{enumerate}
\end{enumerate}
...
\begin{solution}[ex:quad_eq_allgemein_params]
\begin{itemize}
...
\end{itemize}
\end{solution}
\begin{solution}[ex:quad_eq_allgemein_einfluss_params]
...
\begin{solution}[ex:quad_eq_allgemein_einfluss_param_c]
\begin{enumerate}
...
\begin{solution}[ex:quad_func_zeichnen]
\begin{IEEEeqnarray*}{rCrClCrCrCl}
f &:& \Reals & \rightarrow & \Reals
& \quad &
g &:& \Reals & \rightarrow & \Reals
...
\resizebox{0.7\textwidth}{!}{
\begin{tikzpicture}[>=Stealth, scale=0.5]
%wlabels, xlabel, ylabel, xmin, xmax, ymin, ymax
    \drawcustomgrid{x}{f(x)}{-6}{5}{-4}{6}
    \node[circle,draw,inner sep=1pt, orange!80!black, fill] (sp) at (-1,-3) {};
    %\node at ($(sp) + (0.5,-1)$) {\scriptsize {\color{orange!80!black} Scheitelpunkt}};
    \node[circle,draw,inner sep=1pt, blue!80!black, fill] (fp) at (0,-2) {};
    \node[circle,draw,inner sep=1pt, blue!80!black, fill] (mfp) at (-2,-2) {};
    \node[circle,draw,inner sep=1pt, green!70!black, fill] (scnd) at (1,1) {};
    \node[circle,draw,inner sep=1pt, green!70!black, fill] (mscnd) at (-3,1) {};
    \node[circle,draw,inner sep=1pt, red!80!black, fill] (thrd) at (2,6) {};
    \node[circle,draw,inner sep=1pt, red!80!black, fill] (mthrd) at (-4,6) {};
	% symmetry axis    
    \draw[thin, black!60!white, dash dot] (-1,-4) -- (-1,6) {};
    \draw[thin, black!60!white, dashed] (fp) -- (mfp) {};
    \draw[thin, black!60!white, dashed] (scnd) -- (mscnd) {};
    \draw[thin, black!60!white, dashed] (thrd) -- (mthrd) {};

	\clip (-6, -4) rectangle (5,6);
    \draw[thick, smooth, black] plot[domain=-4.5:4.5, samples=100] (\x, {(\x+1)^2-3});
    
    \draw[thin, green!70!black, dashed] (sp) -- (1,-3) node[below, midway] {\footnotesize $\Delta_x$};
    \draw[thin, green!70!black, dashed] (1,-3) -- (scnd) node[right, midway] {\footnotesize $\Delta_y = \Delta_x^2$};
\end{tikzpicture}}
\end{center}
\item Die Funktion $g$ ist in der allgemeinen Form gegeben. Wir bestimmen zuerst ihre Scheitelpunktform durch quadratische Ergänzung:
\begin{IEEEeqnarray*}{rCl}
g(t) &=& -t^2 + 2t -3\\
&=& (-t^2 + 2t -1 )- 2\\
&=& -(t^2 -2t + 1) - 2\\
&=& -(t-1)^2 - 2\\
\end{IEEEeqnarray*}
Somit hat die Funktion $g$ ihren Scheitelpunkt in $(1,-2)$.
\begin{center}
\resizebox{0.5\textwidth}{!}{
\begin{tikzpicture}[>=Stealth, scale=0.5]
%wlabels, xlabel, ylabel, xmin, xmax, ymin, ymax
    \drawcustomgrid{t}{g(t)}{-3}{5}{-11}{2}
    \node[circle,draw,inner sep=1pt, orange!80!black, fill] (sp) at (1,-2) {};
    %\node at ($(sp) + (0.5,-1)$) {\scriptsize {\color{orange!80!black} Scheitelpunkt}};
    \node[circle,draw,inner sep=1pt, blue!80!black, fill] (fp) at (2,-3) {};
    \node[circle,draw,inner sep=1pt, blue!80!black, fill] (mfp) at (0,-3) {};
    \node[circle,draw,inner sep=1pt, green!70!black, fill] (scnd) at (3,-6) {};
    \node[circle,draw,inner sep=1pt, green!70!black, fill] (mscnd) at (-1,-6) {};
    \node[circle,draw,inner sep=1pt, red!80!black, fill] (thrd) at (4,-11) {};
    \node[circle,draw,inner sep=1pt, red!80!black, fill] (mthrd) at (-2,-11) {};
	% symmetry axis    
    \draw[thin, black!60!white, dash dot] (1,-11) -- (1,2) {};
    \draw[thin, black!60!white, dashed] (fp) -- (mfp) {};
    \draw[thin, black!60!white, dashed] (scnd) -- (mscnd) {};
    \draw[thin, black!60!white, dashed] (thrd) -- (mthrd) {};

	
    \draw[thin, green!70!black, dashed] (sp) -- (3,-2) node[above, midway] {\footnotesize $\Delta_t$};
    \draw[thin, green!70!black, dashed] (3,-2) -- (scnd) node[right, midway] {\footnotesize $\Delta_y = -\Delta_t^2$};
	\clip (-3, -11) rectangle (5,2);
    \draw[thick, smooth, black] plot[domain=-4.5:4.5, samples=100] (\x, {-(\x)^2 + 2*\x - 3});
...
\newpage
\subsection{Zu Kapitel 4}

\begin{solution}[ex:identify_quad_eqs]
\begin{multicols}{2}
\begin{enumerate}[label=\alph*)]
\item quadratisch;
...
\begin{solution}[ex:complete_square_solve_eqs]
%\begin{multicols}{2}
\begin{enumerate}[label=\alph*)]
\item \begin{IEEEeqnarray*}{RrCl}
&y^{2}-3y+2&=&0\\
\iff &\left(y^{2}-2\frac{3}{2}y\right) + 2 &=&0\\
\iff &\left(y^{2}-2\frac{3}{2}y + \frac{3^2}{2^2}\right) - \frac{3^2}{2^2}+ 2 &=&0\\
\iff &\left(y- \frac{3}{2}\right)^2 - \frac{1}{4} &=&0\\
\iff &\left(y- \frac{3}{2}\right)^2 &=& \frac{1}{4} \\
\iff &y- \frac{3}{2} &=& \pm \frac{1}{2} \\
\iff &y &=& \frac{3}{2}\pm \frac{1}{2}\\
\Rightarrow & y &\in & \lbrace 1, 2\rbrace
\end{IEEEeqnarray*}
\item  \begin{IEEEeqnarray*}{RrCl}
& 5y^{2} &=&20\\
\iff & y^2 &=& 4\\
\iff & y &=& \pm 2\\
\Rightarrow & y &\in & \lbrace -2, 2\rbrace\\
\end{IEEEeqnarray*}

\item \begin{IEEEeqnarray*}{RrCl}
&2y^{2}-y-6&=&0\\
\iff & y^2 - \frac{1}{2}y - 3 &=& 0\\
\iff & y^2 - 2\frac{1}{4}y - 3 &=& 0\\
\iff & y^2 - 2\frac{1}{4}y + \frac{1}{4^2} - \frac{1}{4^2} - 3 &=& 0\\
\iff & (y-\frac{1}{4})^2 - \frac{1+48}{16} &=& 0\\
\iff &  (y-\frac{1}{4})^2 &=& \frac{49}{16}\\
\iff & y-\frac{1}{4} &=& \pm \frac{7}{4}\\
\Rightarrow & y &\in & \left\lbrace 2, -\frac{3}{2}\right\rbrace \\
\end{IEEEeqnarray*}
\item  \begin{IEEEeqnarray*}{RrCl}
&2y^{2}+y-6&=&0\\
\iff & y^2 + \frac{1}{2}y - 3 &=& 0\\
...
\begin{solution}[ex:solve_quad_eq_formula]
\begin{enumerate}[label=\alph*)]
\item  \begin{IEEEeqnarray*}{RrCl}
&y^{2}-3y+2 &=& 0\\
\Rightarrow & y_{1,2} &=& \frac{-(-3) \pm \sqrt{(-3)^2 - 4\cdot 1 \cdot 2}}{2\cdot 1}\\
\Rightarrow & y_{1,2} &=& \frac{3 \pm \sqrt{9 - 8}}{2}\\
\Rightarrow & y &\in & \left\lbrace 2, 1 \right\rbrace\\
\end{IEEEeqnarray*}
\item  \begin{IEEEeqnarray*}{RrCl}
&5y^{2}&=&20\\
\iff &5y^{2} - 20 &=& 0\\
\Rightarrow & y_{1,2} &=& \frac{-0 \pm \sqrt{0^2 - 4\cdot 5 \cdot (-20)}}{2\cdot 5}\\
\Rightarrow & y_{1,2} &=& \frac{\pm \sqrt{20 \cdot 20}}{10}\\
\Rightarrow & y_{1,2} &=& \frac{\pm 20 }{10}\\
\Rightarrow & y &\in & \left\lbrace 2, -2 \right\rbrace\\
\end{IEEEeqnarray*}
\item  \begin{IEEEeqnarray*}{RrCl}
& 2y^{2}-y-6 &=& 0\\
\Rightarrow & y_{1,2} &=& \frac{-(-1) \pm \sqrt{(-1)^2 - 4\cdot 2 \cdot (-6)}}{2\cdot 2}\\
\Rightarrow & y_{1,2} &=& \frac{1 \pm \sqrt{1+48}}{4}\\
\Rightarrow & y_{1,2} &=& \frac{1 \pm 7}{4}\\
\Rightarrow & y &\in & \left\lbrace 2, -\frac{3}{2} \right\rbrace\\
\end{IEEEeqnarray*}
\item  \begin{IEEEeqnarray*}{RrCl}
& 2y^{2}+y-6 &=& 0\\
...
\begin{solution}[ex:lueckentext_quad_eq_diskriminante]
Es sei die quadratische Gleichung $ax^2 + bx + c=0$ und es sei $\Delta=b^{2}-4ac$. 
\begin{enumerate}[label=\alph*)]
\item Wenn $\Delta>0$, dann gibt es genau 2 Lösungen.
\item Wenn $\Delta=0$, dann gibt es genau 1 Lösungen.
\item Wenn $\Delta<0$, , dann gibt es genau 0 Lösungen.
\end{enumerate}
Die Lösungsformel für die quadratische Gleichung lautet $x_{1,2} = \frac{-b\pm\sqrt{b^2 - 4ac}}{2a} = \frac{-b\pm\sqrt{\Delta}}{2a}$ (siehe auch Gleichung~\ref{eq:quad_eq_general_sol}). 

Wenn $\Delta=0$ gilt, dann ist die einzige Lösung also $\frac{-b}{2a}$. Wenn $\Delta<0$, dann ist der Term $\sqrt{\Delta}$ keine reelle Zahl und die Gleichung hat keine reelle Lösung.
\end{solution}


\begin{solution}[ex:quad_gleich_system]
Da $-2$ eine Lösung zur Gleichung $x^{2}-17x+c=0$ ist, finden wir:
\begin{IEEEeqnarray*}{RrCl}
& (-2)^{2}-17\cdot (-2)+c &=& 0\\
\Rightarrow & 4 + 34 + c &=& 0\\
\Rightarrow & c &=& -38\\
\end{IEEEeqnarray*}
Wir setzen dies in die zweite Gleichung ein und nutzen die Formel aus Gleichung~\ref{eq:quad_eq_general_sol}, um die Lösungsmenge zu bestimmen.

\begin{IEEEeqnarray*}{RrCl}
& x^2 + 13x + c &=& 0\\
\overset{c=-38}{\Rightarrow} & x^2 + 13x -38 &=& 0\\
\Rightarrow & x_{1,2} &=& \frac{-13 \pm \sqrt{13^2 -4\cdot 1 \cdot (-38)}}{2\cdot 1}\\
& &=& \frac{-13 \pm \sqrt{321}}{2}\\
\end{IEEEeqnarray*}
\end{solution}



\begin{solution}[ex:quad_gleich_gemeinsame_loesung]
Die Lösungen zur Gleichung $6x^{2}-17x+12=0$ lauten
\begin{IEEEeqnarray*}{rCl}
x_{1,2} &=& \frac{17 \pm \sqrt{(-17)^2 - 4 \cdot 6 \cdot 12}}{2 \cdot 6}\\
&=& \frac{17 \pm \sqrt{289-288}}{12}\\
&=& \frac{3}{2} \text{ oder } \frac{4}{3}
\end{IEEEeqnarray*}

Wenn $x=\frac{3}{2}$ eine Lösung der Gleichung $3x^{2}-2x+c=0$ sein soll, dann muss gelten:
\begin{IEEEeqnarray*}{RrCl}
& 3\left(\frac{3}{2}\right)^{2}-2\left(\frac{3}{2}\right)+c &=& 0\\
\Rightarrow  & \frac{27}{4} - 3 + c &=& 0\\
\Rightarrow & c &=& -\frac{15}{4}\\
\end{IEEEeqnarray*}

Wenn $x=\frac{4}{3}$ eine Lösung der Gleichung $3x^{2}-2x+c=0$ sein soll, dann muss gelten:
\begin{IEEEeqnarray*}{RrCl}
& 3\left(\frac{4}{3}\right)^{2}-2\left(\frac{4}{3}\right)+c &=& 0\\
\Rightarrow  & \frac{16}{3} - \frac{8}{3} + c &=& 0\\
\Rightarrow & c &=& -\frac{8}{3}\\
\end{IEEEeqnarray*}
Das heisst, die Gleichungen und $6x^{2}-17x+12=0$ und $3x^{2}-2x+c=0$ haben eine gemeinsame Lösung, wenn $c\in \left\{ -\frac{15}{4}, -\frac{8}{3}\right\}$ gilt.
\end{solution}




\begin{solution}[ex:vieta]
\begin{enumerate}[label=\alph*)]
\item Wir finden: \begin{IEEEeqnarray*}{RrCl}
&x_{1,2} &=& \frac{-p \pm \sqrt{p^2 - 4q}}{2}\\
\Rightarrow &x_1 \cdot x_2 &=& \frac{-p + \sqrt{p^2 - 4q}}{2} \cdot \frac{-p - \sqrt{p^2 - 4q}}{2}\\
\Rightarrow &x_1 \cdot x_2 &=& \frac{p - \sqrt{p^2 - 4q}}{-2} \cdot \frac{p + \sqrt{p^2 - 4q}}{-2}\\
\Rightarrow &x_1 \cdot x_2 &=& \frac{\left(p - \sqrt{p^2 - 4q}\right)\left(p + \sqrt{p^2 - 4q}\right)}{\left(-2\right)\cdot \left(-2\right)}\\
\Rightarrow &x_1 \cdot x_2 &=& \frac{p^2 -\left(p^2 - 4q\right)}{4}\\
\Rightarrow &x_1 \cdot x_2 &=& q\\
&x_1 + x_2 &=& \frac{-p + \sqrt{p^2 - 4q}}{2}  + \frac{-p - \sqrt{p^2 - 4q}}{2}\\
&x_1 + x_2 &=& \frac{-2p}{2}\\
&x_1 + x_2 &=& -p\\
\end{IEEEeqnarray*}

\item Es gibt viele Möglichkeiten, solche Gleichungen zu finden. Wenn wir die Formeln aus a) nutzen, finden wir:
\begin{enumerate}[label=\roman*)]
\item $-1$ und $4 \Rightarrow q = (-1) \cdot 4 = -4$ und $p = -(-1+4) = -3$, also $x^2 -3p -4 = 0$
\item $-2$ und $2 \Rightarrow q = (-2) \cdot 2 = -4$ und $p = -(-2 + 2) = 0$, also $x^2 -4 = 0$
\item $3$ und $3 \Rightarrow q = (3 \cdot 3) = 9$ und $p=-(3+3) = -6$, also $x^2 -6x +9 = 0$
\end{enumerate}
Alternativ lässt sich auch argumentieren, dass ein Produkt zweier Zahlen genau dann Null ist, wenn eine oder beide Faktoren Null sind. Und wir wissen, dass Gleichungen der Form $(x-r_1)(x-r_2) = 0$ quadratische Gleichungen sind, da  $(x-r_1)(x-r_2)  = x^2 - x(r_1+r_2) + r_1\cdot r_2$ gilt. Somit lassen sich z.B. auch folgende Lösungen finden:
\begin{enumerate}[label=\roman*)]
\item $(x+1)(x-4)=0$;
\item $(x+2)(x-2)=0$;
\item $(x-3)^2=0$.
\end{enumerate}
\emph{Bemerken Sie Ähnlichkeiten zwischen den beiden Lösungswegen?}
\item François Aussage ist korrekt, denn es gilt:
\begin{IEEEeqnarray*}{rCl}
ax^2 + bx + c &=& 0\\
x^2 + \frac{b}{a}x + \frac{c}{a} &=& 0
\end{IEEEeqnarray*}
Dies entspricht der quadratischen Gleichung aus Aufgabe a) für $p=\frac{b}{a}$ und $q=\frac{c}{a}$. Aus Aufgabe a) wissen wir deshalb, dass gilt:
\begin{IEEEeqnarray*}{c}
\alpha \cdot \beta = -p \Rightarrow \alpha \cdot \beta = -\frac{b}{a}\\
\alpha + \beta = q \Rightarrow \alpha + \beta = \frac{c}{a}.\\
\end{IEEEeqnarray*}
\item Wir wissen, dass das Produkt der beiden Linearterme $(x-\alpha)$ und $(x-\beta)$ genau dann Null ist, wenn $x=\alpha$ oder $x=\beta$ gilt. Wir wissen ausserdem, dass das Produkt zweier Linearterme einen quadratischen Term ergibt. Somit wissen wir, dass $a(x-\alpha)(x-\beta) = 0$ einer quadratischen Gleichung mit Lösungen in $x=\alpha$ und $x=\beta$ entspricht. Wir können den Term aus multiplizieren:

\begin{IEEEeqnarray*}{RrCl}
& a(x-\alpha)(x-\beta) &=& 0\\
\iff & a(x^2 - x(\alpha + \beta) + \alpha \cdot \beta) &=& 0\\
\iff & ax^2 - a(\alpha + \beta)x + a \alpha \cdot \beta &=& 0\\
\end{IEEEeqnarray*}
Wenn $\alpha$ und $\beta$ Beziehungen (i) und (ii) erfüllen, dann finden wir:
\begin{IEEEeqnarray*}{RrCl}
& a(x-\alpha)(x-\beta) &=& 0\\
\iff &ax^2 - a(\alpha + \beta)x + a \cdot \left(\alpha \cdot \beta \right)&=& 0\\
\overset{\alpha + \beta = -\frac{b}{a}, \alpha \cdot \beta = \frac{c}{a}}{\iff} & ax^2 - a(-\frac{b}{a})x + a \frac{c}{a} &=& 0\\
\iff & ax^2 + bx + c &=& 0,\\
\end{IEEEeqnarray*}
...
\begin{solution}[ex:vieta_applied]
Aus Aufgabe~\ref{ex:vieta} wissen wir, dass die Lösungen $\alpha, \beta$ der quadratischen Gleichung $ax^{2}+bx+c=0$ folgende Beziehungen erfüllen müssen:
\begin{enumerate}[label=\roman*)]
\item $\alpha+\beta=-\frac{b}{a}$
\item $\alpha\cdot \beta=\frac{c}{a}.$
\end{enumerate}

Wir wissen ausserdem, dass $\frac{1}{\alpha}$ und $\frac{1}{\beta}$ genau dann Lösungen der quadratischen Gleichung $cx^{2}+bx+a=0$ sind, wenn
\begin{enumerate}[label=\roman*)]
\item $\frac{1}{\alpha}+\frac{1}{\beta}=-\frac{b}{c}$
\item $\frac{1}{\alpha}\cdot \frac{1}{\beta}=\frac{a}{c}$
\end{enumerate}
gilt.
Wir finden:
\begin{IEEEeqnarray*}{rCl}
...
&=& -\frac{b}{c},\\
\end{IEEEeqnarray*}
womit Beziehung $(i)$ erfüllt ist. Ausserdem finden wir:
\begin{IEEEeqnarray*}{rCl}
\frac{1}{\alpha}\cdot \frac{1}{\beta} &=&  \frac{1}{\alpha \cdot \beta}\\
&=& \frac{1}{\frac{c}{a}}\\
&=& \frac{a}{c},\\
\end{IEEEeqnarray*}
womit beide Beziehungen erfüllt sind und bewiesen ist, dass $\frac{1}{\alpha}$ und $\frac{1}{\beta}$ Lösungen zur Gleichung $cx^{2}+bx+a=0$ sind.
\end{solution}
